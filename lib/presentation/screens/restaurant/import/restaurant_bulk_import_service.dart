...
/// Uses store pattern via service locator for reactive state management
class RestaurantBulkImportService {
  // Access stores via service locator convenience getters
  ...
  Future<void> _importCategories(List<List<dynamic>>? rows, ImportResult result) async {
    ...
    for (int i = 1; i < rows.length; i++) {
      try {
        ...
        var existing = categoryStore.categories.where((c) => c.id == id).firstOrNull;
        if (existing != null) {
  ...
  Future<void> _importVariants(List<List<dynamic>>? rows, ImportResult result) async {
    ...
    for (int i = 1; i < rows.length; i++) {
      try {
        ...
        // Check if already exists
        var existing = variantStore.variants.where((v) => v.id == id).firstOrNull;
        if (existing != null) {
  ...
  Future<void> _importExtras(List<List<dynamic>>? rows, ImportResult result) async {
    ...
    for (int i = 1; i < rows.length; i++) {
      try {
        ...
        // Check if already exists
        var existing = extraStore.extras.where((e) => e.Id == id).firstOrNull;
        if (existing != null) {
  ...
  Future<void> _importToppings(List<List<dynamic>>? rows, ImportResult result) async {
    ...
    for (var extraId in toppingGroups.keys) {
      try {
        // Find the extra
        var extra = extraStore.extras.where((e) => e.Id == extraId).firstOrNull;
        if (extra == null) {
          ...
          if (isContainSize && toppingRows.length > 1) {
            ...
            for (var row in toppingRows) {
              ...
              if (variantId.isNotEmpty) {
                ...
                // Get variant model
                var variantModel = variantStore.variants
  ...
  Future<void> _importChoices(List<List<dynamic>>? rows, ImportResult result) async {
    ...
    for (int i = 1; i < rows.length; i++) {
      try {
        ...
        // Check if already exists
        var existing = choiceStore.choices.where((c) => c.id == id).firstOrNull;
        if (existing != null) {
  ...
  Future<void> _importChoiceOptions(List<List<dynamic>>? rows, ImportResult result) async {
    ...
    for (var choiceId in optionGroups.keys) {
      try {
        // Find the choice
        var choice = choiceStore.choices.where((c) => c.id == choiceId).firstOrNull;
        if (choice == null) {
  ...
  Future<void> _importItems(List<List<dynamic>>? rows, ImportResult result) async {
    ...
    for (int i = 1; i < rows.length; i++) {
      try {
        ...
        // Check if already exists
        var existing = itemStore.items.where((item) => item.id == id).firstOrNull;
        if (existing != null) {
  ...
  Future<void> _importItemVariants(List<List<dynamic>>? rows, ImportResult result) async {
    ...
    for (var itemId in variantGroups.keys) {
      try {
        // Find the item
        var item = itemStore.items.where((i) => i.id == itemId).firstOrNull;
        if (item == null) {
