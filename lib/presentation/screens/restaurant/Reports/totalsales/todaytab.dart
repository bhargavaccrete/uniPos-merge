...
class _TodaytabState extends State<Todaytab> {
  ...
  List<pastOrderModel> _getTodaysOrders() {
    final now = DateTime.now();

    return pastOrderStore.pastOrders.where((order) {
  ...
  double _calculateTotalSales(List<pastOrderModel> orders) {
    // Calculate totals from the filtered list (exclude refunded amounts)
    return orders.fold(0.0, (sum, order) => sum + (order.totalPrice - (order.refundAmount ?? 0.0)));
  }

  int _calculateOrderCount(List<pastOrderModel> orders) {
    return orders.where((order) => order.orderStatus != 'FULLY_REFUNDED').length;
  }

  // --- Export to Excel (CSV) Logic ---
  Future<void> _exportToExcel() async {
    final todaysOrders = _getTodaysOrders();
  ...
  Widget build(BuildContext context) {
    return Observer(
      builder: (_) {
        ...
        final todaysOrders = _getTodaysOrders();
        final totalSales = _calculateTotalSales(todaysOrders);
        final orderCount = _calculateOrderCount(todaysOrders);
